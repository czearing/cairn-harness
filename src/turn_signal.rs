...
impl TurnSignal {
    ...
    pub async fn wait_after(&mut self, start: u64) -> Result<TurnEvents> {
        while self.events.recv().await.is_some() {
            if let Some(events) = read_turn(&self.file, start)? {
                return Ok(events);
            }
        }
...
fn read_turn(file: &PathBuf, start: u64) -> Result<Option<TurnEvents>> {
        ...
        match event["type"].as_str() {
            Some("assistant.message") => {
                if let Some(content) = event["data"]["content"].as_str()
                    && !content.contains("HARNESS_SESSION_READY")
                {
            ...
            Some("assistant.turn_end") => ended = true,
            _ => {}
        }
    }
    Ok(ended.then(|| TurnEvents {
        text: output.join("\n"),
        tools,
    }))
}
